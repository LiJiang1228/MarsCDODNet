...
class SpatioTemporalLSTMCell(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, kernel_size=(3, 3)):
        ...
        self.x_projection = PeriodicConv2d(input_dim, 7 * hidden_dim, kernel_size)
        self.h_projection = PeriodicConv2d(hidden_dim, 4 * hidden_dim, kernel_size)
        self.m_projection = PeriodicConv2d(hidden_dim, 3 * hidden_dim, kernel_size)
        self.output_gate = nn.Conv2d(2 * hidden_dim, hidden_dim, kernel_size=1)
        self.output_state = nn.Conv2d(2 * hidden_dim, hidden_dim, kernel_size=1)

    def forward(self, x, state):
        hidden, cell, memory = state
        x_parts = torch.chunk(self.x_projection(x), 7, dim=1)
        h_parts = torch.chunk(self.h_projection(hidden), 4, dim=1)
        m_parts = torch.chunk(self.m_projection(memory), 3, dim=1)

        update = torch.sigmoid(x_parts[0] + h_parts[0])
        forget = torch.sigmoid(x_parts[1] + h_parts[1] + 1.0)
        cell = forget * cell + update * torch.tanh(x_parts[2] + h_parts[2])

        memory_update = torch.sigmoid(x_parts[3] + m_parts[0])
        memory_forget = torch.sigmoid(x_parts[4] + m_parts[1] + 1.0)
        memory = memory_forget * memory + memory_update * torch.tanh(x_parts[5] + m_parts[2])

        combined = torch.cat((cell, memory), dim=1)
        output = torch.sigmoid(x_parts[6] + h_parts[3] + self.output_gate(combined))
        hidden = output * torch.tanh(self.output_state(combined))
...
class PredRNNModel(nn.Module):
    ...
    def __init__(
        ...
        for index, hidden_dim in enumerate(self.hidden_dims):
            ...
            self.encoder_cells.append(
                SpatioTemporalLSTMCell(encoder_channels, hidden_dim, kernel_size)
            )
            self.decoder_cells.append(
                SpatioTemporalLSTMCell(decoder_channels, hidden_dim, kernel_size)
            )
    ...
    def forward(
        ...
        if len(set(self.hidden_dims)) != 1:
            raise ValueError(
                "PredRNN requires equal hidden dimensions so its shared "
                "spatiotemporal memory can pass between layers"
            )

        states = [
            cell.init_state(batch, (height, width), x_dynamic.device)[:2]
            for cell in self.encoder_cells
        ]
        shared_memory = torch.zeros(
            batch, self.hidden_dims[0], height, width, device=x_dynamic.device
        )
        for step in range(self.input_timesteps):
            layer_input = sequence[:, step]
            next_states = []
            for index, cell in enumerate(self.encoder_cells):
                hidden, memory, shared_memory = cell(
                    layer_input, (*states[index], shared_memory)
                )
        ...
        for step in range(self.output_timesteps):
            ...
            for index, cell in enumerate(self.decoder_cells):
                ...
                next_states.append((hidden, memory))
                layer_input = self.dropout(hidden) if index < len(self.decoder_cells) - 1 else hidden
            states = next_states
...
MarsCDODPredRNNModel = PredRNNModel
...
if __name__ == "__main__":
    ...
    reference_model = PredRNNModel(**reference_config)
