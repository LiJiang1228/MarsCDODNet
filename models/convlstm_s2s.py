...
class StandardConvLSTMCell(nn.Module):
    """Vanilla ConvLSTM cell without hidden-state normalization."""
    ...
    def forward(self, x, state):
        hidden, memory = state
        update, forget, output, candidate = torch.chunk(
...
class ConvLSTMS2SModel(nn.Module):
    ...
    def __init__(
        ...
        for index, hidden_dim in enumerate(self.hidden_dims):
            ...
            self.encoder_cells.append(StandardConvLSTMCell(encoder_channels, hidden_dim, kernel_size))
            self.decoder_cells.append(StandardConvLSTMCell(decoder_channels, hidden_dim, kernel_size))
...
MarsCDODStandardConvLSTMModel = ConvLSTMS2SModel
...
if __name__ == "__main__":
    ...
    reference_model = ConvLSTMS2SModel(**reference_config)
