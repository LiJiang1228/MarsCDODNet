...
class ConvGRUCell(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_size=(3, 3)):
        ...
        self.gates = PeriodicConv2d(input_dim + hidden_dim, 2 * hidden_dim, kernel_size)
        self.candidate = PeriodicConv2d(input_dim + hidden_dim, hidden_dim, kernel_size)

    def forward(self, x, hidden):
        update, reset = torch.chunk(
            torch.sigmoid(self.gates(torch.cat((x, hidden), dim=1))), 2, dim=1
        )
    ...
    def init_state(self, batch_size, spatial_size, device):
        return torch.zeros(batch_size, self.hidden_dim, *spatial_size, device=device)
...
class ConvGRUModel(nn.Module):
    ...
    def __init__(
        ...
        for index, hidden_dim in enumerate(self.hidden_dims):
            ...
            self.encoder_cells.append(ConvGRUCell(encoder_channels, hidden_dim, kernel_size))
            self.decoder_cells.append(ConvGRUCell(decoder_channels, hidden_dim, kernel_size))
    ...
    def forward(
        ...
        for cell in self.encoder_cells:
            hidden = cell.init_state(batch, (height, width), x_dynamic.device)
            outputs = []
            for step in range(self.input_timesteps):
                hidden = cell(sequence[:, step], hidden)
        ...
        for step in range(self.output_timesteps):
            ...
            for index, cell in enumerate(self.decoder_cells):
                hidden = cell(layer_input, states[index])
...
MarsCDODConvGRUModel = ConvGRUModel
...
if __name__ == "__main__":
    ...
    reference_model = ConvGRUModel(**reference_config)
